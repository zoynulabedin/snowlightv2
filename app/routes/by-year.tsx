...
export default function ByYearPage() {
  ...
  const genres = [
    "all",
    ...currentYearData.topGenres.map((g) => g.toLowerCase()),
  ];

  const filteredTracks =
    selectedGenre === "all"
      ? currentYearData.popularTracks
      : currentYearData.popularTracks.filter(
          (track) => track.genre.toLowerCase() === selectedGenre
        );
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-8">
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
            <div className="flex items-center space-x-4">
              <Calendar className="w-5 h-5 text-Snowlight-pink" />
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200">
          <div className="p-6 border-b border-gray-200">
            <h3 className="text-xl font-bold text-gray-900">
              {selectedYear}년{" "}
              {selectedGenre === "all"
                ? t("byYear.popularTracks", "인기 트랙")
                : `${selectedGenre.toUpperCase()} ${t(
                    "byYear.tracks",
                    "트랙"
                  )}`}
            </h3>
              ...
              >
                <div className="flex items-center space-x-4">
                  ...
                  <div className="flex items-center space-x-2">
                    <button className="p-2 text-gray-400 hover:text-Snowlight-pink hover:bg-pink-50 rounded-md transition-colors">
        ...
        <div className="mt-8 bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
            {currentYearData.topGenres.map((genre, index) => (
              ...
              >
                <div className="text-2xl font-bold text-Snowlight-pink mb-1">
                  #{index + 1}
                </div>
